...
def extract2d_and_cleanup(data):
    ...
    in_contact = np.array(data['all_contact'])[:,16]
...
def extract_training_data(data):    
    ...
    for i in xrange(start_index, end_index+1-sub, sub):
        if in_contact[i] and in_contact[i+sub]:
...
def main(argv):
    ...
    global rate
    rate = int(argv[2])
    filelist = glob.glob("%s/multipush_shape=rect1_surface=plywood_rep=0000_step=%.04f.json" % (argv[1], rate/100000.0))
    all_training_data = []
    ...
    outputfile_mat= "%s/data_training_vhgp_%d.mat" % (argv[1], rate)
    prepare_and_save_mat(all_training_data, outputfile_mat)
    
    outputfile= "%s/data_training_vhgp_%d.json" % (argv[1], rate)
